...
all: build
	@echo "bloombox-product is ready."
...
build: $(TARGET) $(ENV) dependencies
	@mkdir -p $(TARGET)/
	@cp -frv ./*.html ./bower.json ./behaviors ./README.md $(TARGET)/;
	@echo "Project build complete."

ifeq ($(BUILDBOT),yes)
test:
	wct --job-name "bloombox-product" --expanded --build-number "$(BUILD_NUMBER)" --sauce-tunnel-id "$(SAUCE_TUNNEL)" --plugin wct-jenkins
else
test:
	@wct --job-name "bloombox-product"
endif

quickbuild:
	@echo "Quickbuilding..."
	@cp -fr ./*.html ./bower.json ./behaviors ./README.md $(TARGET)/;
